function newSolicitude() {
    ...
    //Validacion
    $('#idestimate').numeric(); // validamos solo numero
    $('.register_solicitude').on('click', function (e) {
        ...
        var title = $('#idtitle');
        var estimate = $('#idestimate');
        var client = $('.project');
        if (!title.val()) {
            title.attr("placeholder", "No se ha ingresado Titulo.");
            title.addClass("error-incomplete");
            validate = 1;
        }
        if (!estimate.val()) {
            estimate.attr("placeholder", "No se ha ingresado Presupuesto.");
            estimate.addClass("error-incomplete");
            validate = 1;
        }
        if (!client.val()) {
            client.attr("placeholder", "No se ha ingresado Presupuesto.");
            client.addClass("error-incomplete");
        ...
        setTimeout(function () {
            ...
            //validando que solo se ingrese los datos de la lista
            $('.project').each(function (index) {
                var input = $(this).val();

                clients_input[index] = input;// guarda todos los inputs clientes
                input = input.split('-');
                input = $.trim(input[1]);
        ...
        setTimeout(function () {
            // Validando duplicados de las familias
